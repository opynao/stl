...
// LinkedList is faster than std::forward_list
...
TEST(Speed, Test)
{
    auto f1 = []()
    {
        ...
        for (int i = 0; i != ELEMENTS_COUNT; ++i)
            list.InsertAfter(list.GetHead(), i);
    };
    auto f2 = []()
    {
    ...
    std::cerr << "LinkedList<int> runtime=" << profiler<>::duration(f1).count() << "ms\n";
    std::cerr << "std::forward_list<int> runtime=" << profiler<>::duration(f2).count() << "ms\n";
}